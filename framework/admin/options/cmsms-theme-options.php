...
function get_custom_all_meta_fields() {
	...
	$cmsms_heading_after_header = (isset($cmsms_option[CMSMS_SHORTNAME . '_heading_position']) && $cmsms_option[CMSMS_SHORTNAME . '_heading_position'] == 'after_header') ? 'true' : 'false';
	...
	if ( 
		...
		$custom_page_new_meta_fields = array();
		
		
		foreach ($custom_page_meta_fields as $custom_page_meta_field) {
			if ( 
				$custom_page_meta_field['id'] == 'cmsms_content_under_header' && 
				$cmsms_heading_after_header == 'true' 
			) {
				unset($custom_page_meta_field);
			} else {
				$custom_page_new_meta_fields[] = $custom_page_meta_field;
			}
		}
		
		
		$custom_all_meta_fields = array_merge($custom_page_new_meta_fields, $custom_new_meta_fields);
	} elseif ( 
...
function cmsms_admin_enqueue_scripts($hook) {
	if ( 
		...
		wp_localize_script('cmsms_theme_options_js', 'cmsms_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'pet-rescue'), 
			'select_format' => 		esc_attr__('Please select the format.', 'pet-rescue'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'pet-rescue'), 
			'want_remove' => 		esc_html__('Do you realy want to remove this item?', 'pet-rescue'), 
...
function show_cmsms_meta_box() {
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'pet-rescue') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'pet-rescue') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'pet-rescue') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'pet-rescue') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'pet-rescue') . '</option>';
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsms_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'pet-rescue') . '" />' . 
					'<a href="#" class="cmsms_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'pet-rescue') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'pet-rescue') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'pet-rescue') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . __('Blank Field', 'pet-rescue') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'pet-rescue') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . __('Posts', 'pet-rescue') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . __('Pages', 'pet-rescue') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . __('Projects', 'pet-rescue') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'pet-rescue') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'pet-rescue') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'pet-rescue') . ' &nbsp;</option>';
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsms_gallery_item">' . 
							...
							'<a href="#" class="cmsms_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'pet-rescue') . '"></a>' . 
						'</li>';
					}
				}
			}
