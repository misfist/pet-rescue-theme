...
function cmsms_get_settings() {
	...
	case CMSMS_PAGE_BASENAME:
		...
		$cmsms_settings_page_title = esc_html__('Theme General Settings', 'pet-rescue');	
	...
	case CMSMS_PAGE_BASENAME . '-style':
		...
		$cmsms_settings_page_title = esc_html__('Theme Appearance', 'pet-rescue');
	...
	case CMSMS_PAGE_BASENAME . '-font':
		...
		$cmsms_settings_page_title = esc_html__('Theme Fonts Settings', 'pet-rescue');
	...
	case CMSMS_PAGE_BASENAME . '-color':
		...
		$cmsms_settings_page_title = esc_html__('Theme Color Schemes', 'pet-rescue');
	...
	case CMSMS_PAGE_BASENAME . '-single':
		...
		$cmsms_settings_page_title = esc_html__('Theme Single Posts Settings', 'pet-rescue');
	...
	case CMSMS_PAGE_BASENAME . '-demo':
		...
		$cmsms_settings_page_title = esc_html__('Theme Settings Import/Export', 'pet-rescue');
...
function cmsms_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		__('Default Field', 'pet-rescue'), 
		'desc' => 		__('This is a default description.', 'pet-rescue'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function cmsms_settings_scripts() {
	...
	wp_localize_script('cmsms_theme_settings_js', 'cmsms_setting', array( 
		...
		'remove_sidebar' => 	esc_html__('Do you realy want to remove this sidebar?', 'pet-rescue'), 
		'find' => 				esc_attr__('Find icons', 'pet-rescue'), 
		'remove_icon' => 		esc_html__('Do you realy want to remove this social icon?', 'pet-rescue'), 
		'theme_uri' => 			get_template_directory_uri(), 
		'shortname' => 			CMSMS_SHORTNAME, 
		'done' => 				esc_html__('All theme settings were imported successfully.', 'pet-rescue'), 
		'fail' => 				esc_html__("Error on page!!!\nPlease reload page and try again.", 'pet-rescue'), 
...
function cmsms_add_menu() {
	...
	$add_menu_p( 
		...
		esc_attr__('Theme Settings', 'pet-rescue'), 
		esc_attr__('Theme Settings', 'pet-rescue'), 
		'manage_options', 
		CMSMS_PAGE_BASENAME, 
	...
	$cmsms_settings_general = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme General Settings', 'pet-rescue'), 
		esc_attr__('General', 'pet-rescue'), 
		'manage_options', 
		CMSMS_PAGE_BASENAME, 
		'cmsms_settings_page_fn' 
	);
	...
	$cmsms_settings_style = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Appearance', 'pet-rescue'), 
		esc_attr__('Appearance', 'pet-rescue'), 
		'manage_options', 
	...
	$cmsms_settings_font = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Fonts Settings', 'pet-rescue'), 
		esc_attr__('Fonts', 'pet-rescue'), 
		'manage_options', 
	...
	$cmsms_settings_color = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Color Schemes', 'pet-rescue'), 
		esc_attr__('Colors', 'pet-rescue'), 
		'manage_options', 
	...
	$cmsms_settings_single = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Single Posts Settings', 'pet-rescue'), 
		esc_attr__('Single Posts', 'pet-rescue'), 
		'manage_options', 
	...
	$cmsms_settings_demo = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Settings Import/Export', 'pet-rescue'), 
		esc_attr__('Import/Export', 'pet-rescue'), 
		'manage_options', 
...
function cmsms_settings_page_fn() {
	...
	if ($current_tab != 'wpml') {
		...
		'<p class="submit-top">' . 
			(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'pet-rescue') . '" />' : '') . 
		'</p>';
		...
		echo '<p class="submit">' . 
				(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'pet-rescue') . '" />' : (($current_tab == 'import') ? '<input name="import" type="button" class="button button-primary button-large cmsms-demo-import" value="' . esc_attr__('Import Settings', 'pet-rescue') . '" />' : '')) . 
			'</p>' . 
		'</form>';
	} else {
		do_settings_sections(__FILE__);
...
function cmsms_form_field_fn($args = array()) {
	...
	case 'rgba':
		...
		'<br />' . 
		'<small>' . esc_html__('Color opacity', 'pet-rescue') . ':</small>' . 
		'<div class="cl"></div>' . 
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'pet-rescue') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'pet-rescue') . '</label>' . 
			'</div>';
		}
		...
		if ($font_color) {
			echo '<div class="cmsms_admin_block" style="padding-bottom:20px;">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_color" style="padding-bottom:15px;">' . esc_html__('Font Color', 'pet-rescue') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsms_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'pet-rescue') . ' <em>(' . __('in pixels', 'pet-rescue') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsms_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'pet-rescue') . ' <em>(' . __('in pixels', 'pet-rescue') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'pet-rescue') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'pet-rescue') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'pet-rescue') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'pet-rescue') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'pet-rescue') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsms_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'pet-rescue') . '" />' . 
				'<a href="#" class="cmsms_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'pet-rescue') . '"></a>' . 
			'</p>' . 
			...
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_link">' . 
					...
					__('Icon Link', 'pet-rescue') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					__('Icon Title', 'pet-rescue') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					__('Open link in a new tab/window?', 'pet-rescue') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'pet-rescue') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'pet-rescue') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'pet-rescue') . '"></a> ' . 
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : __('Expecting a Numeric value!', 'pet-rescue');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						CMSMS_SHORTNAME . '_txt_numeric_error',
						__('Expecting a Numeric value! Please fix.', 'pet-rescue'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values', 'pet-rescue');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						...
						__('Expecting comma separated numeric values! Please fix.','pet-rescue'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : __('Invalid email! Please re-enter!', 'pet-rescue');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.', 'pet-rescue');
				}
				...
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						...
						__('Please enter a valid email address.', 'pet-rescue'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : __('Number!', 'pet-rescue');
...
function theme_admin_bar_render() {
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Theme Settings', 'pet-rescue'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('General', 'pet-rescue'), 
		'href' => admin_url('admin.php?page=cmsms-settings') 
	));
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Appearance', 'pet-rescue'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Fonts', 'pet-rescue'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Colors', 'pet-rescue'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Single', 'pet-rescue'), 
