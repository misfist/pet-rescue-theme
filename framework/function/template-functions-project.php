		...
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_cat']) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'pet-rescue') . ':' . '</div>' . 
...
function cmsms_project_comments($template_type = 'page', $show = true) {
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = '<a class="cmsms_project_comments cmsms_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'pet-rescue') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>';
		} elseif ($template_type == 'post') {
			$cmsms_option = cmsms_get_global_options();
			$out = '';
...
function cmsms_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'pet-rescue') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function cmsms_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsms_project_author">' . 
			...
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'pet-rescue') . ' ' . get_the_author_meta('display_name') . '" class="vcard author"><span class="fn" rel="author">' . get_the_author_meta('display_name') . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsms_option = cmsms_get_global_options();
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'pet-rescue') . ':' . '</div>' . 
				'<div class="project_details_item_desc vcard author">' . 
					'<span class="cmsms_project_author fn" title="' . esc_attr__('Projects by', 'pet-rescue') . ' ' . get_the_author_meta('display_name') . '" rel="author">' . get_the_author_meta('display_name') . '</span>' . 
				'</div>' . 
			'</div>';
		}
	}
	...
	if (get_the_terms($cmsms_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_tag']) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Tags', 'pet-rescue') . ':' . '</div>' . 
					'<div class="project_details_item_desc">' . 
...
function cmsms_project_link($link_text, $link_url, $link_target, $show = true) {
	...
	if ( 
		...
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'pet-rescue') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
				'<span class="cmsms_project_tags">' . 
